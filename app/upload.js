...
const UploadScreen = () => {
  ...
  floodImgurl = "get_flood_image";
  floodVidurl = "get_flood_video";
  ...
  const [FloodshowBarChart, setFloodShowBarChart] = useState(true);
  const toggleFloodChart = () => {
    setFloodShowBarChart(!FloodshowBarChart);
  };
    ...
    if (selectedValue) {
      ...
      if (type == "Flood") {
        if (selectedValue && pic && img == 1) {
          ...
          const apiUrl = `http://192.168.104.24:5000/flood/image?email=${email}&city=${selectedValue}`;
        ...
        if (selectedValue && pic && img == 2) {
          ...
          const apiUrl = `http://192.168.104.24:5000/flood/video?email=${email}&city=${selectedValue}`;
  ...
  const [FloodbarData, setFloodbarData] = useState([
    { value: 0, label: "AH" },
    { value: 0, label: "AP", frontColor: "#177AD5" },
    { value: 0, label: "DC", frontColor: "#177AD5" },
  ]);

  const [Floodseries, setFloodSeries] = useState([1, 1, 1]);

  useEffect(() => {
    if (floodData.length > 0) {
      const totalDamage = floodData[0]["Total Damage"];
      const totalHouses = floodData[0]["Total Houses Affected"];
      const peopleAffected = floodData[0]["People Affected (Approx)"];
      ...
      if (totalDamage && peopleAffected) {
        ...
        const newFloodBarData = [
          // { value: totalHouses, label: "AH" },
          { value: peopleAffected, label: "AP", frontColor: "#177AD5" },
          { value: totalcost, label: "DC", frontColor: "#177AD5" },
        ];
        // const newFloodSeries = [totalHouses, peopleAffected, floodDamage];
        const newFloodSeries = [ peopleAffected, totalcost];

        setFloodbarData(newFloodBarData);
        setFloodSeries(newFloodSeries);
      }
    }
  }, [floodData]);
        ...
        <View>
          {floodData.length > 0 ? (
            <View style={styles.reportContainer}>
              {img === 1 ? (
                <Image
                  source={{ uri: `http://192.168.104.24:5000/${floodImgurl}?${Math.random()}` }}
                  style={{
                    height: 250,
                    width: "100%",
                    marginBottom: 20,
                ...
                <VideoPlayer
                  videoProps={{
                    ...
                    source: {
                      uri: `http://192.168.104.24:5000/${floodVidurl}?${Math.random()}`
                    },
                  }}
                  style={{ height: 400 }}
                />
              ...
              {floodData.map((item, index) => (
                <View key={index}>
                  ...
                  <View style={styles.row}>
                    ...
                    <Text style={styles.value}>
                      {item["People Affected (Approx)"]}
                    </Text>
                  </View>

                  <View style={styles.row}>
                    <Text style={styles.label}>Damage Cost:</Text>
                    <Text style={styles.value}>{item["Total Damage"]}</Text>
                  </View>
