...
import earthquakeBackground from "../assets/earthbackground.jpg";
...
export const Earthquake = () => {
  ...
  const handleImageUpload = () => {
    if (selectedImage) {
      ...
      axios
        .post(`http://127.0.0.1:5000/earthquake/image?email=${localStorage.getItem('email')}&city=${city}`, formData, {
          headers: {
  ...
  const handleViewHistory = () => {
    axios
      .get(
        `http://127.0.0.1:5000/earthquakedata?email=${localStorage.getItem(
          "email"
        )}&city=${city}`
      )
      .then((response) => {
...
const handleCityChange = (event) => {
  ...
  const handleVideoUpload = () => {
    if (selectedVideo) {
      ...
      axios
        .post(`http://127.0.0.1:5000/earthquake/video?email=${localStorage.getItem('email')}&city=${city}`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        })
        ...
        {earthquakeData ? (
          <div className="flex flex-wrap bg-black/50 overflow-auto justify-center flex-row gap-5 items-center">
           <DisasterInfoCard
            ...
            url={selectedImage!=null ? 'get_earth_image':'get_earth_video' }
            className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4"
          />
          <div className="flex flex-row gap-5 mb-32" >
          <CardWithPieChart data={earthquakeData} className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4" />
          <CardWithBarChart data={earthquakeData} className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4" />
          <CardWithLineChart data={earthquakeData} className="w-full sm:w-1/2 md:w-1/3 lg:w-1/4" />
          </div>
